...
`audio.c` provides minimal register-based SFX for jump, mine, place, craft, and failed craft, plus the lightweight `eyenaut_theme` music loop generated by the piano recorder tool. The theme runs on CH2 and is advanced once per frame from the game loop; most SFX use CH1 or CH4 so the melody keeps playing. This layer is intentionally small so hUGEDriver can take over richer music later without unwinding a large custom driver.
